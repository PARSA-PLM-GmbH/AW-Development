...
export let initialize2DViewer = function( subPanelContext, isReloadViewer, atomicDataRef ) {
    ...
    function _setupViewer( subPanelContext, atomicDataRef ) {
        ...
        if(subPanelContext !== undefined &&  subPanelContext.viewerRef !== undefined && subPanelContext.viewerRef.current !== undefined ){
    ...
    for ( let i = 0; i < navElements.length; ++i ) {
        ...
        for ( let j = 0; j < classList.length; ++j ) {
            if ( classList[j].indexOf( 'aw-layout-infoCommandbar' ) >= 0 ) {
                ...
                // Looking for the command bar down the right side of AW to get the height.
                // If this element doesn't have a height, it is the wrong one.
                if(height <= 0){
                    continue;
                }
                ...
                if ( universalViewerHeight > height ) {
                    ...
                    let viewerElement = document.getElementById( 'aw-2d-viewer' );
                    if( viewerElement !== undefined)
                    {
                        let viewerRect = viewerElement.getBoundingClientRect();
                        let elemRect = navElements[i].getBoundingClientRect();
                        if( elemRect !== undefined && viewerRect !== undefined)
                        {
                            // Get the y position difference between the 2D viewer
                            // and the command bar.  Subtract this from the overall
                            // size so the 2D viewer takes up the available space.
                            yMove = viewerRect.y - elemRect.y;
                        }
                    }
                ...
                // Subtract 75 from height to account for the toolbar
                parentElement.style.height = height + 'px';
                currElement.style.height = height - 75 + 'px';
