 ...
 */
export let processGetAttributeComplexDataResponse = function( parameter, measureValue, response, engrTable, subPanelContext ) {
    ...
    if( subPanelContext.parametersTable ) {
        let selectedParam = cdm.getObject( subPanelContext.parametersTable.selectedObjects[0].props.att1SourceAttribute.dbValue );
        if( !( selectedParam.type === 'Att0MeasurableAttributeBool' || selectedParam.type === 'Att0MeasurableAttributePnt' ) ) {
            let propertyDisplayNames = Att1EngrTableService.getGMMMDisplayNames( subPanelContext.parametersTable.selectedObjects[0] );
            engrTableCtx.propertyDisplayNames = propertyDisplayNames;
        }
    }else{
        if( !( subPanelContext.openedObject.type === 'Att0MeasurableAttributeBool' || subPanelContext.openedObject.type === 'Att0MeasurableAttributePnt' ) ) {
            let propertyDisplayNames = Att1EngrTableService.getEngrDisplayPropNames( subPanelContext.openedObject );
            engrTableCtx.propertyDisplayNames = propertyDisplayNames;
        }
    }
