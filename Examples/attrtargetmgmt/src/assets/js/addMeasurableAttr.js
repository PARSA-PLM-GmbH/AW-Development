...
export let prepareInputForCreateParametersSoa = function( data, editHandler ) {
    ...
    if( editHandler ) {
        ...
        if( dataSource ) {
            let modifiedViewModelProperties = dataSource.getAllModifiedProperties();
            _.forEach( modifiedViewModelProperties, function( vmProp ) {
                if( vmProp &&  ( vmProp.propertyName ===  'att0PointX' || vmProp.propertyName ===  'att0PointY' || vmProp.propertyName ===  'att0PointZ' ) || vmProp && vmProp.dbValue !== null && vmProp.dbValue !== '' || vmProp.uiValue !== '' && vmProp.dbValue !== ''   ) {
