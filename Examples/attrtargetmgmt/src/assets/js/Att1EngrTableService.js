 ...
 * @returns {*} property display names
 */
export let getGMMMDisplayNames = ( selectedParam )=>{
    return {
...
let localizeHeaders = function( tableData, data ) {
    let selectedParam = data.selectedParameter ?? data.engrTable.selectedParameter;
    if( selectedParam.type === 'Att0MeasurableAttributeBool' || selectedParam.type === 'Att0MeasurableAttributePnt' ) {
        clientLocalization( tableData, data );
    } else{
        ...
        if( data.engrTable.propertyDisplayNames ) {
            ...
            propertyDisplayNames = getGMMMDisplayNames( data.subPanelContext.parametersTable.selectedObjects[0] );
        }
...
let clientLocalization = ( tableData, data )=>{
    var headers = {};
    if( data && data.i18n ) {
        tableData.subHeaders.forEach( header => {
            headers[ header ] = data.i18n[ header ] ? data.i18n[ header ] : header;
        } );
...
export let updateSelection = function( engrTable, cells, dragType, minCol, maxCol ) {
    ...
    var canEnableColumnFillCommand = isEnableColumnFillCommand( cells, tableName );
 ...
 */
function isEnableColumnFillCommand( cells, tableName ) {
...
export let getParameterComplexData = function( subPanelContext, data, importCompleted ) {
    ...
    if( !selectedObject && subPanelContext.parametersTable ) {
        selectedObject = subPanelContext.parametersTable.selectedUnderlyingObjects;
    }
    if( selectedObject ) {
        attrTableProp = selectedObject[ 0 ].props.att0AttributeTable;
        m_selectedParameter = subPanelContext.parametersTable.selectedUnderlyingObjects[ 0 ];
    ...
    var isImportOrAddTableInProgress = !importCompleted && subPanelContext.parametersTable &&
        ( subPanelContext.parametersTable.isComplexDataImportInProgress || subPanelContext.parametersTable.addNewTableValueCommandClicked );
